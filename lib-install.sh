...
DRACULASUR_SOURCE+=("lib-install.sh")
...
install_beggy() {
  ...
  case "${background}" in
    blank)
      cp -r "${THEME_SRC_DIR}/assets/gnome-shell/common-assets/background-blank.png"          "${DRACULASUR_TMP_DIR}/beggy.png" ;;
    default)
      if [[ "${no_blur}" == "false" && "${no_darken}" == "true" ]]; then
        cp -r "${THEME_SRC_DIR}/assets/gnome-shell/common-assets/background-blur.png"         "${DRACULASUR_TMP_DIR}/beggy.png"
      elif [[ "${no_blur}" == "false" && "${no_darken}" == "false" ]]; then
        cp -r "${THEME_SRC_DIR}/assets/gnome-shell/common-assets/background-blur-darken.png"  "${DRACULASUR_TMP_DIR}/beggy.png"
      elif [[ "${no_blur}" == "true" && "${no_darken}" == "true" ]]; then
        cp -r "${THEME_SRC_DIR}/assets/gnome-shell/common-assets/background-default.png"      "${DRACULASUR_TMP_DIR}/beggy.png"
      else
        cp -r "${THEME_SRC_DIR}/assets/gnome-shell/common-assets/background-darken.png"       "${DRACULASUR_TMP_DIR}/beggy.png"
      fi
      ;;
    *)
      if [[ "${no_blur}" == "false" || "${darken}" == "true" ]]; then
        install_beggy_deps
        convert "${background}" ${CONVERT_OPT}                                                "${DRACULASUR_TMP_DIR}/beggy.png"
      else
        cp -r "${background}"                                                                 "${DRACULASUR_TMP_DIR}/beggy.png"
      fi
      ;;
  esac
}
...
install_darky() {
  ...
  sassc ${SASSC_OPT} "${THEME_SRC_DIR}/main/gtk-3.0/gtk-dark.scss"                            "${DRACULASUR_TMP_DIR}/darky-3.css"
  sassc ${SASSC_OPT} "${THEME_SRC_DIR}/main/gtk-4.0/gtk-dark.scss"                            "${DRACULASUR_TMP_DIR}/darky-4.css"
}
...
install_shelly() {
  ...
  cp -r "${DRACULASUR_TMP_DIR}/beggy.png"                                                       "${TARGET_DIR}/assets/background.png"
...
install_theemy() {
  ...
  local TMP_DIR_T="${DRACULASUR_TMP_DIR}/gtk-3.0${color}${opacity}${alt}${theme}"
  local TMP_DIR_F="${DRACULASUR_TMP_DIR}/gtk-4.0${color}${opacity}${alt}${theme}"
  ...
  desktop_entry+="CursorTheme=DraculaSur-cursors\n"
...
install_gdm_theme() {
  ...
  # Let's go!
  install_theme_deps
  rm -rf "${DRACULASUR_GS_DIR}"; install_beggy
  gtk_base "${colors[0]}" "${opacities[0]}" "${themes[0]}"

  if check_theme_file "${COMMON_CSS_FILE}"; then # CSS-based theme
    install_shelly "${colors[0]}" "${opacities[0]}" "${alts[0]}" "${themes[0]}" "${icon}" "${DRACULASUR_GS_DIR}"
    sed $SED_OPT "s|assets|${DRACULASUR_GS_DIR}/assets|" "${DRACULASUR_GS_DIR}/gnome-shell.css"
    ...
    ln -sf "${DRACULASUR_GS_DIR}/gnome-shell.css" "${COMMON_CSS_FILE}"
    ln -sf "${DRACULASUR_GS_DIR}/gnome-shell.css" "${TARGET}"

    # Fix previously installed DraculaSur
    restore_file "${ETC_CSS_FILE}"
  else # GR-based theme
    install_shelly "${colors[0]}" "${opacities[0]}" "${alts[0]}" "${themes[0]}" "${icon}" "${DRACULASUR_TMP_DIR}/shelly"
    sed $SED_OPT "s|assets|resource:///org/gnome/shell/theme/assets|" "${DRACULASUR_TMP_DIR}/shelly/gnome-shell.css"
    ...
    backup_file "${TARGET}"
    glib-compile-resources --sourcedir="${DRACULASUR_TMP_DIR}/shelly" --target="${TARGET}" "${GS_GR_XML_FILE}"
...
install_firefox_theme() {
  ...
  if [[ "${monterey}" == 'true' ]]; then
    ...
    udo cp -rf "${FIREFOX_SRC_DIR}"/DraculaSur/{icons,titlebuttons}                             "${TARGET_DIR}"/Monterey
    ...
    udo cp -rf "${FIREFOX_SRC_DIR}"/DraculaSur                                                  "${TARGET_DIR}"
    udo cp -rf "${FIREFOX_SRC_DIR}"/userChrome-DraculaSur.css                                   "${TARGET_DIR}"/userChrome.css
  fi
