...
elif [[ "${DRACULASUR_SOURCE[@]}" =~ "lib-core.sh" ]]; then
  echo "'lib-core.sh' is already imported"; exit 1
fi

DRACULASUR_SOURCE=("lib-core.sh")
...
export DRACULASUR_TMP_DIR="/tmp/DraculaSur.lock"
...
#--------------GDM----------------#
DRACULASUR_GS_DIR="/usr/share/gnome-shell/theme/DraculaSur"
...
### TODO: return "lockDraculaSur()" back for non functional syntax error handling
###       and lock dir removal after immediate terminal window closing

if [[ -d "${DRACULASUR_TMP_DIR}" ]]; then
  start_animation; sleep 2; stop_animation; echo

  if [[ -d "${DRACULASUR_TMP_DIR}" ]]; then
    prompt -e "ERROR: Whitesur installer or tweaks is already running. Probably it's run by '$(ls -ld "${DRACULASUR_TMP_DIR}" | awk '{print $3}')'"
    exit 1
  fi
fi

rm -rf "${DRACULASUR_TMP_DIR}"
mkdir -p "${DRACULASUR_TMP_DIR}"; exec 2> "${DRACULASUR_TMP_DIR}/error_log.txt"
...
signal_error() {
  ...
  IFS=$'\n'
  local sources=($(basename -a "${DRACULASUR_SOURCE[@]}" "${BASH_SOURCE[@]}" | sort -u))
  local dist_ids=($(awk -F '=' '/ID/{print $2}' "/etc/os-release" | tr -d '"' | sort -Vru))
  local repo_ver=""
  local lines=()
  local log="$(awk '{printf "\033[1;31m  >>> %s\n", $0}' "${DRACULASUR_TMP_DIR}/error_log.txt" || echo "")"
  ...
  for i in "${sources[@]}"; do
    lines=($(grep -Fn "${DRACULASUR_COMMAND:-${BASH_COMMAND}}" "${REPO_DIR}/${i}" | cut -d : -f 1 || echo ""))
    prompt -e "  >>> ${i}$(IFS=';'; [[ "${lines[*]}" ]] && echo " at ${lines[*]}")"
  done

  prompt -e "SNIPPET:\n    >>> ${DRACULASUR_COMMAND:-${BASH_COMMAND}}"
  prompt -e "TRACE  :"
  ...
  prompt -i "https://github.com/vinceliuice/DraculaSur-gtk-theme/issues\n\n"

  rm -rf "${DRACULASUR_TMP_DIR}"; exit 1
}
